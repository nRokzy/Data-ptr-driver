...
namespace CallSpoofer
{
	...
	public:
		uintptr_t temp = 0;
		const uintptr_t xor_key = 0x7a3f8d692eb5c41f;
	...
	{
		void* addr = ExAllocatePoolWithTag(NonPagedPool, size, (ULONG)"NtBg");
		if (!addr)
			return nullptr;
	...
	public:
		...
		__forceinline decltype(auto) operator()(Args&&... args)
		{
			...
			while (orig_generator[index])
			{
				...
				{
#ifdef _KERNEL_MODE
					//print(_("found allocated generator\n"));
#else
					//printf(_("[$] shellcode: found generator\n"));
#endif
			...
			{
#ifdef _KERNEL_MODE
				//print(_("allocating generator\n"));
#else	
				//printf(_("[$] shellcode: alloc generator\n"));
#endif
